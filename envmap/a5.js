...
floorNormalTexture.minFilter = THREE.a;
...
////////////////////// PUDDLE SHADER /////////////////////////////
grassTexture = textureLoader.load("images/grass.jpg");
grassTexture.minFilter = THREE.LinearMipmapNearestFilter;
var puddleMaterial = new THREE.ShaderMaterial({
  uniforms: {
    vcsLightPosition: { value: new THREE.Vector3(0.0, 0.0, -1.0) },
    matrixWorld: { value: new THREE.Matrix4() },
    uNegxTexture: { type: "t", value: negxTexture },
    ...
    uPoszTexture: { type: "t", value: poszTexture },
    normalMap: { type: "t", value: floorNormalTexture },
    textureMap: { type: "t", value: grassTexture },
    myColor: { value: new THREE.Vector4(0.8, 0.8, 0.6, 1.0) },
    time: { value: new Date().getTime() },
  },
  side: THREE.DoubleSide,
  vertexShader: document.getElementById("floorVertShader").textContent,
  fragmentShader: document.getElementById("puddleFragShader").textContent,
});
...
posxMaterial = new THREE.MeshBasicMaterial({
  map: posxTexture,
  side: THREE.DoubleSide,
});
...
negyMaterial = new THREE.MeshBasicMaterial({
  map: negyTexture,
  side: THREE.DoubleSide,
});
...
negzMaterial = new THREE.MeshBasicMaterial({
  map: negzTexture,
  side: THREE.DoubleSide,
});
...
negxMaterial = new THREE.MeshBasicMaterial({
  map: negxTexture,
  side: THREE.DoubleSide,
});
...
posyMaterial = new THREE.MeshBasicMaterial({
  map: posyTexture,
  side: THREE.DoubleSide,
});
...
poszMaterial = new THREE.MeshBasicMaterial({
  map: poszTexture,
  side: THREE.DoubleSide,
});
...
/////////////////////////////////////
// Puddle:  square patch on the ground
/////////////////////////////////////
puddleGeometry = new THREE.PlaneBufferGeometry(10, 10);
puddle = new THREE.Mesh(puddleGeometry, puddleMaterial);
puddle.position.x = 0.0;
puddle.position.z = 13.0;
puddle.position.y = -1.0;
...
function checkKeyboard() {
  ...
  } else if (keyboard.pressed("S")) light.position.y -= 0.1;
  if (keyboard.pressed("A")) light.position.x -= 0.1;
  else if (keyboard.pressed("D")) light.position.x += 0.1;
  ...
  puddleMaterial.uniforms.vcsLightPosition.value = vcsLight;
  puddleMaterial.uniforms.vcsLightPosition.value.needsUpdate = true;
}
...
function update() {
  ...
  puddleMaterial.uniforms.matrixWorld.value = camera.matrixWorld;
  puddleMaterial.uniforms.time.value = new Date().getTime() - initialTime;
  puddleMaterial.uniforms.time.update = true;
  puddleMaterial.uniforms.matrixWorld.update = true;
